package softarch.portal.data;

import java.sql.ResultSet;
import java.sql.SQLException;
import java.text.ParseException;
import java.util.Date;

import javax.servlet.http.HttpServletRequest;

/**
 * Represents an <i>expensive subscription</i> user account.
 * @author Niels Joncheere
 */
public class ExpensiveSubscription extends RegularUser {
	public ExpensiveSubscription(HttpServletRequest request) {
		super(request);
	}
	
	public ExpensiveSubscription(ResultSet rs) throws SQLException, ParseException {
		super(rs);
	}
	
	public ExpensiveSubscription(String username, String password, String firstName, 
			String lastName, String emailAddress, Date lastLogin) 
	{
		super(username, password, firstName, lastName, emailAddress, lastLogin);
	}
	
	@Override
	protected String getType() {
		return "ExpensiveSubscription";
	}
}
